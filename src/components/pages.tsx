    ...
    body: (
      <div className="space-y-4 font-normal">
        ...
        <div className="flex flex-col gap-1">
          ...
          <p className="font-normal text-(--secondary-foreground) text-sm">
  ...
  terms: {
    ...
    body: (
      <div className="space-y-4 font-normal text-sm">
        ...
        <ul className="list-disc text-(--secondary-foreground) pl-5">
  ...
  privacy: {
    ...
    body: (
      <div className="space-y-4 font-normal text-sm">
        ...
        <p className="text-(--secondary-foreground)">
          当サイトは、法令に基づく場合を除き、予めユーザーの同意を得ることなく第三者に個人情報を提供することはありません。ただし、本サービスの基盤としてSupabaseを使用しており、データはSupabaseのサーバー上で管理されます。
        </p>

        <h3 className="font-bold text-(--foreground) mt-6">5. データの削除</h3>
        <p className="text-(--secondary-foreground)">
          ユーザーは設定画面の「データ削除」ボタンから、いつでも自身の全データをサーバーから完全に削除することができます。なお、アクセス解析によって収集された匿名情報については、本操作の対象外となります。
        </p>
      </div>
    ),
  },
      ...
      return (
        <div className="space-y-4 font-normal">
          ...
          <div className="flex flex-col gap-2">
            ...
            <div className="show-scrollbar bg-background/50 rounded-md p-3 max-h-60 overflow-y-auto space-y-1 text-sm">
              {pastLogs.map((log, index) => (
                <div key={index} className="flex flex-col gap-0.5">
                  <div className="flex gap-3 text-(--foreground)">
                    <span className="shrink-0 w-18">{log.date}</span>
                    <p>{log.content}</p>
                  </div>
                  {log.comment && (
                    <p className="text-xs text-(--secondary-foreground) pl-21 leading-relaxed">
                      {log.comment}
                    </p>
                  )}
                </div>
